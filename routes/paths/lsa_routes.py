...
@routes.app.route('/lsa/bioinformatics')
def get_lsa_bio():
    path_to_jsons = courseflow.app.config['COURSE_PATH_JSONS']
...
@routes.app.route('/lsa/dataandinformation')
def get_lsa_data_and_info():
    ...
    return flask.render_template("lsa-ai.html", **data_path_info)

@routes.app.route('/lsa/economicsandcomputation')
def get_lsa_econ():
    path_to_jsons = courseflow.app.config['COURSE_PATH_JSONS']
    with open(f'/{path_to_jsons}/econ-schema.json/') as econ_json:
        econ_info = json.load(econ_json)
    return flask.render_template("lsa-ai.html", **econ_info)

@routes.app.route('/lsa/roboticsandvision')
def get_lsa_robotics():
    path_to_jsons = courseflow.app.config['COURSE_PATH_JSONS']
    with open(f'/{path_to_jsons}/robotics-schema.json/') as robotics_json:
        robotics_info = json.load(robotics_json)
    return flask.render_template("lsa-ai.html", **robotics_info)

@routes.app.route('/lsa/security')
def get_lsa_security():
    path_to_jsons = courseflow.app.config['COURSE_PATH_JSONS']
    with open(f'/{path_to_jsons}/security-schema.json/') as security_json:
        security_info = json.load(security_json)
    return flask.render_template("lsa-ai.html", **security_info)

@routes.app.route('/lsa/softwaredevelopment')
def get_lsa_software_development():
    ...
    return flask.render_template("lsa-ai.html", **software_dev_info)

@routes.app.route('/lsa/softwaresystems')
def get_lsa_software_systems():
    ...
    return flask.render_template("lsa-ai.html", **software_systems_info)

@routes.app.route('/lsa/theoryofcomputation')
def get_lsa_theory():
    path_to_jsons = courseflow.app.config['COURSE_PATH_JSONS']
